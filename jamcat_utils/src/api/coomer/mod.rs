...
use rand::Rng;
use model::{CreatorInfo, PostInfo};
...
impl CoomerApi {
    ...
    pub async fn random_creator(&self) -> Result<CreatorInfo, ApiError> {
        ...
        let random_creator_idx = rand::rng().random_range(0..creators_len);
    ...
    pub async fn get_file_url(&self, file_info: model::FileInfo) -> Result<String, ApiError> {
        let file_url = format!("{URL_SITE}/data/{}?f={}", file_info.path, file_info.name);
