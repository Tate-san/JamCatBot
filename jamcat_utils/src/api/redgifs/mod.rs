...
use model::{GifList, TemporaryAccessResponse};
...
impl RedgifsApi {
    ...
    pub async fn login_temporary(&mut self) -> Result<(), ApiError> {
        ...
        self.client = HttpClientBuilder::new()
    ...
    pub async fn search(
        ...
        match status {
            reqwest::StatusCode::OK => {
                let res = serde_json::from_str::<model::GifList>(&text)?;
                Ok(res)
            }
            _ => {
                let error_message = serde_json::from_str::<model::Error>(&text)?;
