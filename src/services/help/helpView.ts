...
function formatCommand(prefix: string, command: HelpCommandEntry) {
  ...
  const usages = [
    ...
    .join(' / ');
  const params =
    'params' in command && command.params?.length
...
export function findHelpPage(category: string | undefined) {
  ...
  const index = helpCategories.findIndex(
    (item) =>
      item.name.toLowerCase() === normalized || item.name.toLowerCase().startsWith(normalized),
  );

  return index === -1 ? 0 : index;
}

export function buildHelpPage(prefix: string, page = 0, totalPages: number = getHelpPageCount()) {
