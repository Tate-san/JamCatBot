...
pub async fn volume(ctx: Context<'_>, volume: Option<f32>) -> Result<(), Error> {
    ...
    if volume.is_none() {
        if let Some(cache) = ctx.data().guild_cache.lock().await.get(&guild_id) {
            let current_volume = cache.queue.lock().await.volume();
            ctx.send_message(Message::Other(format!("Volume is at {current_volume}%")))
                .await?;
        }
        return Ok(());
    }
    ...
    if has_handler {
        ...
        jamcat_music::set_volume(&ctx, volume).await?;
