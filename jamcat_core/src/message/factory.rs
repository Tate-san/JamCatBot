...
pub fn create_coomer_image_embed(
    creator: &api::coomer::model::CreatorInfo,
    image_url: String,
    creator_url: String,
...
pub fn create_now_playing_embed(info: &TrackInfo, state: &TrackState) -> serenity::CreateEmbed {
    ...
    if let Some(duration) = info.duration {
        ...
        let elapsed_str = jamcat_utils::duration_string(Duration::from_secs(elapsed_secs));
        let time_str = jamcat_utils::duration_string(duration);
        ...
        embed = embed.field(
            "",
            format!("**{}**", jamcat_utils::brainrot::random()),
            false,
        );
    }
...
pub fn create_queue_list_embed(
    ...
    let total_pages = jamcat_utils::get_total_pages(tracks_len, items_per_page).max(1);
    ...
    if tracks_len > 0 {
        ...
        let queue_duration_str = jamcat_utils::duration_string(Duration::from_secs(total_queue_duration_secs));
